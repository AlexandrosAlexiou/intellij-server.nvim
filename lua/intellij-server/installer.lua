...
function M.install(opts)
  ...
  vim.system({ "curl", "-fSL", "--progress-bar", "-o", tmp_file, url }, { text = true }, function(result)
    vim.schedule(function()
      if result.code ~= 0 then
        local msg = "Download failed: " .. (result.stderr or "unknown error")
        vim.notify("[intellij-server] " .. msg, vim.log.levels.ERROR)
        if opts.on_complete then
          opts.on_complete(false, msg)
        end
      ...
      vim.system(
        { "unzip", "-qo", tmp_file, "extension/server/*", "-d", install_dir },
        { text = true },
        function(unzip_result)
          vim.schedule(function()
            ...
            if unzip_result.code ~= 0 then
              local msg = "Extraction failed: " .. (unzip_result.stderr or "unknown error")
              vim.notify("[intellij-server] " .. msg, vim.log.levels.ERROR)
              if opts.on_complete then
                opts.on_complete(false, msg)
              end
